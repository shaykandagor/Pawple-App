      ...
      <TextInput
        ...
        right={
          surfixIcon && (
            <TextInput.Icon
              icon={surfixIcon}
              onPress={onSurfixIconPressed ? onSurfixIconPressed : toggleShowMap}
            />
          )
        }
        left={
          prefixIcon && (
            <TextInput.Icon
              icon={prefixIcon}
              onPress={onPrefixIconPressed ? onPrefixIconPressed : toggleShowMap}
            />
          )
        }
      />
      {(error || helpText) && (
        <HelperText type={error ? 'error' : 'info'}>{error ? error : helpText}</HelperText>
      )}
      {showMap && (
        <Modal animationType="slide" visible={showMap} onRequestClose={toggleShowMap}>
          <View style={{ flex: 1 }}>
            {markerLocation && (
              <MapView
              ...
              >
                <Marker
                  ...
                  title={calloutTitle}
                  description={
                    descriptionExtractor ? descriptionExtractor(markerLocation) : undefined
                  }
                  onCalloutPress={() =>
                    Alert.alert(
                      'Confirmation',
